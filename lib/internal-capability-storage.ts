...
const TIMED_WAKE_USAGE_GUIDE = [
    "以下是你获取指令的返回结果：",
    "动作：稍后主动联系",
    "用途：为当前聊天约定「过一会儿主动联系对方」。结合当下的真实生活场景自主判断合理延时（例如：拍黄焖鸡/快餐/拍照分享约 4~8 分钟；午休便餐约 15~25 分钟；外出游玩/大餐聚会/电影约 60~120 分钟；开会/上课约 40~60 分钟）。到点后系统会把你当时的想法推回上下文，你再决定发消息、发照片还是静默。",
    "",
    "参数：",
    "- delayMinutes (number): 从现在开始多少分钟后到点（由你根据当前事情自主估算合理时长，必须大于 0）",
    "- intent (string): 到点后你想主动做什么 / 想找对方聊什么 / 发什么照片，用一句话说明",
    "",
    "规则：",
    "- 只要你向对方提及了「等下/待会儿/晚点/吃完/到了」联系或发照片，请立即主动调用本指令，把约定记入日程并由系统准时唤醒你。",
    "- delayMinutes 不要死板套用固定数字，根据具体事情的真实生活耗时自主灵活决定。",
